...
void solve() {
	...
	vi base(N, 1);
	SegmentTree sgtkeep(base);
	vi basec(N);
	SegmentTree sgtchange(basec);

	vpii all; for (int i=0; i<n; ++i) all.pb({a[i], i});
	...
	for (pii p: all) {
		...
		// keep a[i]
		int keep = (idx-1 >= 0 ? sgtkeep.query(0, idx-1) : 0) ;

		// change a[i]
		int change = (idx+1 < n ? sgtkeep.query(idx+1, n-1)  : 0);


		if (keep <= change) {
			// ans += keep;
			ans += sgtkeep.query(0, idx-1);
			sgtkeep.update(idx, 0);
		} else {
			// ans += change;
			ans += sgtkeep.query(idx+1, n-1);
			sgtkeep.update(idx, 0);
			sgtchange.update(idx, 1);
		}
	}
